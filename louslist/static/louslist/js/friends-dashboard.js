...
const showFriendButton = (friendID, friendUsername, showAdd) => {
    ...
    if(showAdd){
        ...
        removeButton.remove();
        const addButtonHTML = `<button type="button" id="add-friend-${friendID}" class="btn btn-outline-success" onclick="addFriend(${friendID}, '${friendUsername}')">Add Friend</button>`;
        friendButtonsBody.insertAdjacentHTML("beforeend", addButtonHTML);
    }
    else {
        const addButton = document.getElementById(`add-friend-${friendID}`);
        addButton.remove();
        const removeButtonHTML = `<button type="button" id="remove-friend-${friendID}" class="btn btn-outline-danger" onclick="removeFriend(${friendID}, '${friendUsername}')">Remove Friend</button>`;
